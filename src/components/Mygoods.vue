<template>
	<div class="hot" >
		...
		<van-button icon="replay" type="info" 
		:disabled="isLoading" @click="loadMoreHandle" color="linear-gradient(to right, #4bb0ff, #6149f6)"
		 loading-text="加载中..." :loading="load" :display="active">{{this.butxt}}</van-button>
	</div>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			async getMygoods(n){
				await axios.get("http://192.168.18.65:3000/goods").then(res => {
					this.active = "block"
				this.load = false
				res = res.data;
				if (res.code === 0) {
